...
from app.schemas import pupils as schema_pupils
...
def get_pupil_by_id(db: Session, id: int, owner: Optional[models.User] = None) -> models.Pupil | None:
    ...
    print(f"Getting pupil by id: {id}, owner: {owner.id if owner else 'None'}")
...
def create_pupil(db: Session, pupil: schema_pupils.PupilCreate, owner: models.User) -> models.Pupil:
...
def update_pupil(db: Session, pupil: models.Pupil, updated_pupil: schema_pupils.PupilCreate) -> models.Pupil:
